...
| WP-026 | Settings Audit Trail and Guardrails | in_progress | codex | 2026-02-27 | Migrated to Azure-first audit pipeline (request middleware + EF mutation interceptor + ingestion service), disabled DB-backed settings/access audit writes, kept `GET /api/store-settings/brand/{brandId}/audit-logs` in transitional empty mode, and retained strict settings write guardrails (`RequireBrandAdmin`) |
...
| 2026-02-27 | Implemented Azure audit runtime foundation in API: added request audit middleware, EF mutation interceptor (including legacy POS dynamic context), Azure Logs Ingestion client wiring/config, optional Azure Monitor OTel export, and disabled DB-backed access audit writes in favor of Azure-first audit events. Validation: `dotnet build backend/EWHQ.Api/EWHQ.Api.csproj`, `npm run lint`, and `npm run build` in `frontend-hq-portal`. |
