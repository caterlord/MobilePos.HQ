...
public class SmartCategoriesController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                // SmartCategoryId generated by DB identity
                AccountId = accountId,
    ...
    public async Task<ActionResult> DeleteSmartCategory(int brandId, int smartCategoryId, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Clean up MenuDetail records (built-in + custom menus)
            var menuDetails = await context.MenuDetails
                .Where(d => d.AccountId == accountId && d.IsSmartCategory && toDelete.Contains(d.CategoryId))
                .ToListAsync(cancellationToken);
            context.MenuDetails.RemoveRange(menuDetails);
